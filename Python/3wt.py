...
with open('/home/kumarsu/dol-mainframe-parsing/IBD-3WT-Commissions-011514.txt', 'rt') as f:
    data = f.read()
    f.close()
...
with open('/home/kumarsu/dol-mainframe-parsing/output/sun4.txt','rt') as h:
     input_data = h.read()
     h.close()
#     input_data = h.read()
#     h.close()
...
#    p5 = re.compile('(OFFICE\s+:\s+)([a-zA-Z0-9*$./:\-%@#]+[\s])')
    ...
    if re.search(p8,str1):
       ...
       match_accat = match_accat.rstrip('\n').strip()
    else:
       match_accat = ''
